...
class AccessController {
  static createSessionHandler = async function (
  ...
  static refreshAccessTokenHandler = async function (
    req: Request,
    res: Response
  ) {
    ...
    new OkResponse(
      await AccessService.refreshAccessToken(refreshToken, clientId)
    ).send(res)
  }
  ...
  static signOutHandler = async function (req: Request, res: Response) {
    ...
    new OkResponse(await AccessService.signOut(refreshToken, clientId)).send(
      res
    )
  }
}
